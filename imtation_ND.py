    ...
    def checkBounds(self):
        for i in range (num_args):            
            if self.X[i] < bound[0]:
                ...
                self.X[i]= self.X[i]          
    def simulate_annealing(self, T, num_dots):
        index = set(np.array(range(num_args), int))
        args_to_change = random.sample(index, num_dots)
        x_new = []
        for i in range(num_args):
            if i in args_to_change:
                x_new.append(self.X[i]+ np.random.normal(0, sigma))
            else:
...
while( T > T_min or I<1500):
    ...
    for i in range(pop.population_size):
        new_individ = pop.population[i].simulate_annealing(T,1)
